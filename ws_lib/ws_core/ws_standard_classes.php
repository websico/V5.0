...
class WssPage extends WsTopLevelContainer {
	...
	function WssPage($id=0, $properties=0, $style='') {
		if (!($properties & WS_CONTAINER_ORIENTATION))
			$properties |= WS_VERTICAL_CONTAINER;		// Default orientation = vertical
        parent::WsTopLevelContainer($id, $properties | WS_PAGE_CONTAINER, $style);
        $this->url_id = $this->id;
...
class WssForm extends WsContainer {
	...
	function Display($html_prefix = '', $html_suffix = '') {
        ...
        $action = '';
        // If destination is not a mail address, we assume it's an URL for action
        if (!preg_match("/^[^@]+@[^@]+(\s*,\s*[^@]+@[^@]+)*$/", $this->destination))
            $action = 'action="'.$this->destination.'"';
        $prefix = '<form ';
        $prefix .= $action.' method="post" enctype="multipart/form-data" onsubmit="
				...
				for (i = 0; labels[i]; i++) {
                    ...
                    // Very light check for old browsers
                    var erfield;
        ...
        if (!empty($this->ccPrompt)) {
	        $prefix .= " 
	            ...
	            ccForm.innerHTML =
	                ...
	                + '<br><br><input type=\'email\' multiple=\'multiple\' name=\'ws_mailto\' size=\'25\' value=\'' + ccMail + '\' onfocus=\'this.value=\'\'\'>'
    ...
    function DisplayContent() {
        ...
        // if no explicit submit button is present...
	  	echo '
			...
			<input type="submit" style="position: absolute; opacity: 0; filter: progid:DXImageTransform.Microsoft.Alpha(opacity=0)">
			';
        parent::DisplayContent();
    }

    //  Action is trigged by ws_inits.php at beginning of request analysis
    //  It returns if necessary a message to display before page
    //  ------------------------------------------------------------------
    function Action() {
        global $ws_insecure, $ws_user_lang;
...
class WssRSSReader extends WsComponent {
	...
	function Display($prefix="", $suffix="") {
        ...
        if ($ws_permissions & P_EDIT)
    		parent::Display(
                $prefix.'<article ws_value="'.($this->value ? $this->value : '').'"'
                .($this->display_date ? ' ws_display_date="true"' : '')
                .($this->display_content ? ' ws_display_content="true"' : '')
                .' ws_max_items="'.$this->max_items.'"', "/article>".$suffix);
        else
