...
new ws_Form('ws_pageToolbar', function(){
	...
	this.setup = function(){
	    var toolBar = document.getElementById("ws_pageToolbar");
	    var me = this;
...
//		document.documentElement.addEventListener("mouseenter", function(){toolBar.style.height = height + 'px'});
	...
	this.tools = function(){                        // Onclick admin button
		if (ws_Form.checkPermission(P_ADMIN))
	    	document.location.href = 'ws_tools.html?ws_referer=' + escape(document.location.href);
	};
...
// Output the toolbar in the flow		
document.write($Forms.ws_pageToolbar.HTML);
$Forms.ws_pageToolbar.setup();
