...
//  Furthermore, defining the forms HTML in javascript allows to cache them
//  in the browser cache.

// ELEMENT FORM CLASS
// ----------------------------------------
ws_ElementForm.prototype = new ws_Form;	// Inherit standard ws_Form members

function ws_ElementForm(id, setMembers){

	// Setup the specific form
	// -----------------------
	ws_Form.call(this, id, setMembers);	// Setup standard and my own members
	var onshow = this.onshow;			// Onshow I call my specific onshowForm then my parent's one
	this.onshow = function(){this.onshowForm(); onshow.call(this)};
}

// Build the form HTML
// -------------------
ws_ElementForm.prototype.buildForm = function(formBody, moreControls){
	return ws_Form.prototype.buildForm.call(this,
	    // Common fields
...
// -----------------------------------------------
ws_ElementForm.prototype.onshowForm = function(){
...
// Main buttons and associated handlers
// ------------------------------------
ws_ElementForm.prototype.buttonsHTML = {
    more:
...
new ws_ElementForm('wssrssreader_form', function(){
	this.HTML = this.buildForm(
	  	$Str.rssSource + '<br><input type="text" size="50" name="ws__value">'
			...
			+ '<div><div id="ws_dateControl" class="ws_buttonText inline">' + $Str.rssShowDate + '</div>'
			+ '<div id="ws_contentControl" class="ws_buttonText inline">' + $Str.rssShowContent + '</div></div>'
	);
	...
	this.onshow = function(){
	    ...
	    var f = ws_selectedElement.ws_updateForm;
  		...
  		f.ws__value.value = elt.getAttribute('ws_value');
  		f.ws__max_items.value = elt.getAttribute('ws_max_items');
		var b = new ws_Button({
			id: "ws_dateControl",
			value: elt.getAttribute("ws_display_date"),
			onClick: function(){f.ws__display_date.value = this.value}
		  });
		f.ws__display_date.value = b.value;
		b = new ws_Button({
			id: "ws_contentControl",
			value: elt.getAttribute("ws_display_content"),
