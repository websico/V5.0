...
function ws_Form(id, setMembers){
	// Can be called without args, just for inheritance (cf ws_ElementForm)
	if (arguments.length) {
...
/*	    var disabled = true;
	    if (draftStyles = document.getElementById('user_StyleDraft')) {
	        // Search if draft styles are used by this page, to enable/disable publish styles checkbox
	        draftStyles = draftStyles.innerHTML;
	    	for (var i = 0; i < ws_containerList.length; i++) {
	            if ((style = ws_containerList[i].getAttribute('ws_style'))
	                    && draftStyles.indexOf(ws_containerList[i].getAttribute('ws_class') + '_' + style) != -1) {
	                disabled = false;
	                break;
	            }
	        }
	        if (disabled) {
	        	for (var i = 0; i < ws_componentList.length; i++) {
	                if ((style = ws_componentList[i].getAttribute('ws_style'))
	                        && draftStyles.indexOf(ws_componentList[i].getAttribute('ws_class') + '_' + style) != -1) {
	                    disabled = false;
	                    break;
	                }
	            }
...
new ws_Form('ws_new_page_form', function(){
	...
	// -------------------
	this.submit = function(){
		var f = document.getElementById('ws_pageToolbar');
		f.ws_operation.value = 'new_page';
