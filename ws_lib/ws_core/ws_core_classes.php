...
//  ----------------------
	private function Read($css_id, $draft = 0, $purge = 0) {
		...
		// In TRIAL_PAGE mode, the the stylesheet is derived from original read sheet, for each user
		global $ws_permissions, $ws_session;
		...
		// OLD FASHION UPDATED ***************************************************************************************
			if ($this->properties & WS_CONTAINER_ORIENTATION) { 	// Oriented container (system)
			  	if ($this->properties & WS_PAGE_CONTAINER) {
			  	  	$ws_container_list = array();
			  	  	$ws_setup_components = '';
//					Some issues of height computing appear with a div, so we keep a table (oh my god !!!!) obsolet comment
//			  		echo '<div style="height: 100%; width: auto"'.$ctnr_attr.">\n".'<table class="wssystemcontainer" style="height: 100%">';
//			  		echo '<table id="outerContainer" class="wssystemcontainer" style="height: 100%"><tr><td style="height: 100%"'.$ctnr_attr.">\n".'<table class="wssystemcontainer" style="height: 100%">';
...
//			echo "\n<!-- End of container ".$this->id." -->\n\n";
	  	...
	  	if ($this->properties & WS_PAGE_CONTAINER) {
	  		...
	  		echo "\n<script>";
	  		foreach ($ws_container_list as $ct) {
                echo "\nws_setupElement('".$ct->id
                    ."', ".(WS_CONTAINER | $ct->properties | ($ct->model_id  ? WS_MODEL_CONTAINER : 0))
                    .", '".@$ct->owner->id."', '".@$ct->contents[count($ct->contents)-1]->id."');";
			}
			echo $ws_setup_components;
	  		echo "\n</script>";
	  	}
	}
	...
	function Display($html_prefix="", $html_suffix="") {
	    ...
	    if ($ws_permissions & P_EDIT) {
			...
			if ($this->owner && count($this->owner->contents) == 1)
				...
				$this->properties &= ~WS_ALONE;
			$ws_setup_components .= "\nws_setupElement('".$this->id."', ".$this->properties.", '".$this->owner->id."');";
	    } else {
...
//	---------------
	function DisplayContent() {
	  	if (!empty($this->value) || $this->value == '0')    // '0' is not empty for us !!
			echo ws_bsn2br_special($this->value);
		else
...
//	-------------
	function DisplayContent() {
		...
		if (!empty($this->caption) || $this->caption == '0' || $ws_permissions & P_EDIT) {
			...
			echo ' class="ws_img_caption" id="'.$this->id.'_caption" '.$style.'>'.ws_bsn2br_special($this->caption);
			...
			if (isset($_POST['ws__width']) && $_POST['ws__width'] != $this->width
				...
				if ($type && (!$_POST['ws__keep_format'] || $type == IMAGETYPE_PNG || $type == IMAGETYPE_JPEG)) {
					...
					if ($_POST['ws__width'] < $width) {										
						$extension = $type == IMAGETYPE_PNG && $_POST['ws__keep_format'] ? '.png' : '.jpg';
