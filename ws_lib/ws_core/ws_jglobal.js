...
ws_addWindowHandler("load",
	function(){
		$Gallery = new function(){
			...
			var goodBrowserResize = navigator.userAgent.indexOf('Safari') != -1 ||
			...
			function loadImage(i) {		// Load images
				...
				var imagePath = goodBrowserResize ? gImg.path
			        : WS_CORE_PATH + "ws_buildimage.php?url=" + gImg.path + "&height=" + windowHeight + "&width=" + windowWidth;
		...
		var computedStyle = menu.currentStyle || window.getComputedStyle(menu, null);
	  	if (computedStyle['display'] != 'block') {
			...
			if (!menu.ws_displayed) {
				...
				var parentX = parentItem.getBoundingClientRect().left + scrollX;
				var parentY = parentItem.getBoundingClientRect().top + scrollY;
...
/*			if (scroll == 0 || (scroll > lastScroll && mainToolbar.style.position == 'fixed')) {
		...
		// Add elements bar
/*		if (elementsBar = document.getElementById('ws_elementsBar')){
			var barRect = elementsBar.getBoundingClientRect();
			var barBottom = Math.floor(elementsBar.lastChild.getBoundingClientRect().bottom);
			var barHeight = barBottom - barRect.top;
			var tBarBottom = Math.floor(tBarRect.bottom);
			var windowBottom = document.documentElement.clientHeight;
			if (barHeight > windowBottom - tBarBottom && document.documentElement.className.indexOf('ws_hideMenu') == -1){
				if (elementsBar.style.position != 'fixed'
							&& scroll < lastScroll && barRect.top > tBarBottom){
					elementsBar.style.position = 'fixed';
					elementsBar.style.top = tBarRect.bottom + 'px';
				} else if (elementsBar.style.position != 'fixed'
							&& scroll > lastScroll && barRect.top < tBarBottom && barBottom < windowBottom){
					elementsBar.style.position = 'fixed';
					elementsBar.style.top = windowBottom - barHeight + 'px';
				} else if (elementsBar.style.position == 'fixed'
							&& ((scroll < lastScroll && barBottom <= windowBottom))
								|| (scroll > lastScroll && barRect.top >= tBarBottom)){
					elementsBar.style.position = 'absolute';
					elementsBar.style.top = scroll + elementsBar.offsetTop + 'px';
				}
			}
		}*/ 
		// Bubble elements
