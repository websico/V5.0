...
//	------------------------------------------------------------------------
if (!($ws_permissions & P_ADMIN) || (isset($_REQUEST['subsite']) &&
														$ws_site_name != 'websico'
														&& stripos($_REQUEST['subsite'], $ws_site_name.'.') !== 0))
    header("Location: .");          // Back home silently if not authorized

if (isset($_REQUEST['subsite'])) {	// Work with a subsite
	unset($ws_norobots);
	unset($ws_noFleximage);
	unset($ws_dont_explore);
	$ws_bashkroun = '';
	@include ($site->path."ws_preferences.php");
    $ws_site = new WsSite($_REQUEST['subsite']);
 ...
 */'."\n";
	...
	unset($ws_norobots);						// Unset check before reloading
	unset($ws_noFleximage);
    ...
    if ($ws_langs_array != $old_langs_array
            ...
            && @mysql_select_db("ws_".$ws_site_name)) {
...
<a id="infoButton" href="http://help.websico.net/'.$ws_lang.'/man_admin.html" target=_blank title="Documentation"><img src="'.WS_CORE_PATH.'ws_images/question.png"></a>
...
<button title="'.$ws_exit_caption.'" id="exit_from_tools" href=""
        onclick="document.location.href =\''.(@$ws_session->referer ? $ws_session->referer : '/').'\'; return false">
	'.$ws_exit_caption.'</button>';
echo '
...
<div class="title">'.$ws_responsive_text.'</div>
...
//  Software management
//  -------------------
if (is_dir('../../ws_lib_ref')) {	// For administered versions
...
if (function_exists('symlink') && !WINDOWS
		&& $ws_site->offer != 'Free' && $ws_site->offer != 'Personal' && $ws_site->offer != 'Standalone') {
    echo '
    <div class="group">
